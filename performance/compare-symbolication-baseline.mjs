...
if (process.argv.length < 4 || process.argv.length > 5) {
  console.error("usage: node performance/compare-symbolication-baseline.mjs <baseline.json> <candidate.json> [max-regression-percent]");
  process.exit(2);
}
...
for (const artifact of [baseline, candidate]) {
  if (artifact.schema_version !== 1 || artifact.metadata?.scenario !== "symbolication-disabled-baseline-phase-6" || !artifact.metrics) {
    throw new Error("unsupported or incomplete Symbolication baseline artifact");
  }
}
for (const field of ["hardware", "rust_toolchain", "fixture", "backend"]) {
...
console.log(`baseline RPS: ${baseline.metrics.rps} -> ${candidate.metrics.rps} (${change.toFixed(2)}%)`);
const failures = [];

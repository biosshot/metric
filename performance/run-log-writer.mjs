...
const metrics = output.match(
  /LogWriter Phase 24: rps=(\d+),records=(\d+),batches=(\d+),average_occupancy=([\d.]+),concurrency=(\d+),elapsed_ms=(\d+)/,
);
if (!metrics) {
  throw new Error("benchmark output did not match the Phase 24 Log writer schema");
}
...
const path =
  process.argv[2] ??
  `performance/results/log-writer-${new Date().toISOString().replaceAll(":", "-")}.json`;

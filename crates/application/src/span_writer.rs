//! Bounded durable Transaction/Span micro-batching.
...
const SPAN_RECORD_OVERHEAD_BYTES: usize = 224;

#[derive(Debug, Clone, Copy)]
pub struct SpanWriterConfig {
...
impl Default for SpanWriterConfig {
    fn default() -> Self {
        Self {
...
pub enum SpanWriterStartError {
    #[error("SpanWriter configuration is invalid")]
...
impl Batch {
    ...
    fn can_push(&self, command: &Command, config: SpanWriterConfig) -> bool {
...
impl SpanWriter {
    pub fn start(
        ...
        config: SpanWriterConfig,
        shutdown: ShutdownSignal,
    ) -> Result<(Arc<Self>, SpanWriterTask), SpanWriterStartError> {
    ...
    fn persist_spans(
        ...
        Box::pin(async move {
            ...
            let records = records
                .into_iter()
                .map(|record| {
                    let estimated_bytes = estimated_span_bytes(&record);
...
fn validate_config(config: SpanWriterConfig) -> Result<(), SpanWriterStartError> {
    let valid = config.channel_capacity > 0
        && (1..=500).contains(&config.max_documents)
        && config.max_bytes >= SPAN_RECORD_OVERHEAD_BYTES
    ...
    if valid {
        ...
        Err(SpanWriterStartError::InvalidConfiguration)
    }
}

fn estimated_span_bytes(record: &SpanRecord) -> usize {
    [
...
mod tests {
    ...
    fn record(byte: u8, body_bytes: usize) -> SpanRecord {
        let project_id = ProjectId::new(42).unwrap();
        let trace_id = TraceId::parse("00112233445566778899aabbccddeeff").unwrap();
        let mut span_bytes = [0_u8; 8];
        span_bytes[7] = byte.max(1);
        ...
        SpanRecord {
            ...
            status: "ok".into(),
            name: format!("GET /span/{byte}").into(),
            environment: None,
            release: None,
            service: Some("writer-test".into()),
            insight_flags: 0,
            body: SignalBody::new(vec![byte; body_bytes]),
        }
    }

    fn config() -> SpanWriterConfig {
        SpanWriterConfig {
    ...
    async fn timer_flushes_a_partial_batch() {
        ...
        let port: Arc<dyn SignalStore> = store.clone();
        let (writer, task) = SpanWriter::start(port, config(), root.signal()).unwrap();
        assert_eq!(
            writer.persist_spans(vec![record(1, 8)]).await,
    ...
    async fn concurrent_records_are_micro_batched() {
        ...
        for byte in 1..=100 {
            ...
            requests.push(tokio::spawn(async move {
                writer.persist_spans(vec![record(byte, 8)]).await
            }));
        }
    ...
    async fn byte_limit_splits_batches() {
        ...
        let store = Arc::new(FakeStore::new(Duration::ZERO));
        let port: Arc<dyn SignalStore> = store.clone();
        let (writer, task) = SpanWriter::start(
            port,
            SpanWriterConfig {
                max_bytes: SPAN_RECORD_OVERHEAD_BYTES + 128,
                ..config()
            },
            root.signal(),
        )
        ...
        let first = {
            let writer = Arc::clone(&writer);
            tokio::spawn(async move { writer.persist_spans(vec![record(1, 32)]).await })
        };
        assert_eq!(
            writer.persist_spans(vec![record(2, 32)]).await,
            Ok(vec![DurableOutcome::Accepted])
        );
    ...
    #[tokio::test]
    async fn full_lane_rejects_without_waiting() {
        let root = crate::shutdown::ShutdownRoot::new();
        let store = Arc::new(FakeStore::new(Duration::from_millis(100)));
        let port: Arc<dyn SignalStore> = store.clone();
        let (writer, task) = SpanWriter::start(
            port,
            SpanWriterConfig {
        ...
        let first = {
            let writer = Arc::clone(&writer);
            tokio::spawn(async move { writer.persist_spans(vec![record(1, 8)]).await })
        };
        store.started.notified().await;
        let second = {
            let writer = Arc::clone(&writer);
            tokio::spawn(async move { writer.persist_spans(vec![record(2, 8)]).await })
        };
        tokio::task::yield_now().await;
        assert_eq!(
            writer.persist_spans(vec![record(3, 8)]).await,
    ...
    async fn shutdown_drains_and_rejects_new_records() {
        ...
        let (writer, task) = SpanWriter::start(store, config(), root.signal()).unwrap();
        let queued = {
            let writer = Arc::clone(&writer);
            tokio::spawn(async move { writer.persist_spans(vec![record(1, 8)]).await })
        };
        ...
        assert!(queued.is_ok() || matches!(queued, Err(SignalStoreError::Unavailable)));
        assert_eq!(
            writer.persist_spans(vec![record(2, 8)]).await,
    ...
    #[ignore = "Phase 25 Span writer RPS baseline runs explicitly in release mode"]
    async fn performance_span_writer_rps_and_batch_occupancy() {
        ...
        for start in (0..RECORDS).step_by(CONCURRENCY) {
            ...
            for index in start..(start + CONCURRENCY).min(RECORDS) {
                ...
                requests.push(tokio::spawn(async move {
                    writer
                        .persist_spans(vec![record(((index % 255) + 1) as u8, 128)])
                        .await
                }));
            }
            for request in requests {
        ...
        eprintln!(
            "SpanWriter Phase 25: rps={rps:.0},records={durable},batches={},average_occupancy={average_occupancy:.2},concurrency={CONCURRENCY},elapsed_ms={}",
            batches.len(),
            elapsed.as_millis()
        );
        assert_eq!(durable, RECORDS);
        assert!(rps >= 20_000.0, "Span writer {rps:.0} RPS is below gate");
        assert!(
            average_occupancy >= 100.0,
            "average Span batch occupancy {average_occupancy:.2} is below gate"
        );
    }
}
